...
export function RepoRow({ repo, index }: RepoRowProps) {
  return (
    ...
    >
      {/* Status dot */}
      <span aria-hidden="true" className="pt-[3px] text-[8px] leading-none text-status-dot group-hover:text-warm transition-colors select-none">
        ◆
      </span>
      ...
      <div className="min-w-0">
        ...
        >
          ...
          <span className="font-bold text-foreground break-all">{repo.name}</span>
        </Link>
        ...
        <div className="relative z-10 flex items-center gap-1.5 mt-2.5 flex-wrap">
      ...
      <p className="m-0 hidden md:block pt-[2px] text-[12.5px] leading-relaxed text-muted-foreground line-clamp-2">
        {repo.description}
      </p>

      {/* Branch column (md+) */}
      <span className="hidden md:block pt-[2px] text-[12px] text-muted-foreground truncate">
        {repo.branch}
      </span>

      {/* Updated */}
      <span className="pt-[2px] text-[11px] md:text-[12px] tabular-nums text-dim text-right whitespace-nowrap">
        {repo.updatedAt}
      </span>
    </li>
  );
